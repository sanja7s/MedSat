...
def prepare_lsoa_GP_population(mappings_dir, year=None):
    """
    Load the LSOA patient population mapping for a specific year.
...
def load_lsoa_mappings(mappings_dir, year=None):
    ...
    Returns:
        Dictionary mapping years to LSOA distribution mappings
    """
...
def calculateTemporalMetrics_LSOA(all_presc, mappings_dir, year=None):
    """
    ...
    Args:
        ...
        mappings_dir: Directory containing mapping files
        year: Year of the data (for selecting appropriate LSOA mapping)
    ...
    # Load appropriate LSOA mapping based on year and format
    if year is not None:
        LSOA_map = load_lsoa_mappings(mappings_dir, year)
    else:
        # Determine the year from the format
        if fields['format'] == 'old':
            LSOA_map = load_lsoa_mappings(mappings_dir, 'old')
        else:
            LSOA_map = load_lsoa_mappings(mappings_dir, '2021')
...
def writeResultFiles(monthly_borough_quantity_new, monthly_borough_dosage_new, monthly_borough_costs_new, 
                     monthly_borough_items_new, diseases, output_dir, mappings_dir, year=None):
    """
    ...
    LSOA_patient_pop = prepare_lsoa_GP_population(mappings_dir, year)
...
def calculateTemporalMetrics_LSOA_opioids(all_presc, mappings_dir, year=None):
    """
...
def writeResultFiles_opioid(monthly_borough_quantity_new, monthly_borough_dosage_new, monthly_borough_costs_new, 
                            monthly_borough_items_new, diseases, output_dir, mappings_dir, year=None):
    """
